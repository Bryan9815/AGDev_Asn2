...
bool CLuaInterface::Init(const char* luaFilePath)
{
	Drop();
	bool result = false;
	
	char FilePath[80];
	sprintf(FilePath, "%s", luaFilePath);
	...
	if (theLuaState)
	{
		...
		// 3. Load lua script
		luaL_dofile(theLuaState, FilePath);
...
void CLuaInterface::saveIntValue(const char* filePath, const char* varName, const int value, const bool bOverwrite)
{
	lua_getglobal(theLuaState, "SaveToLuaFile");
	char file[80];
	sprintf(file, "%s", filePath);
	lua_pushstring(theLuaState, file);
...
void CLuaInterface::saveFloatValue(const char* filePath, const char* varName, const float value, const bool bOverwrite)
{
	lua_getglobal(theLuaState, "SaveToLuaFile");
	char file[80];
	sprintf(file, "%s", filePath);
